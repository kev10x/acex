...
const publicLimiter = rateLimit({ windowMs: 15 * 60 * 1000, max: 60, standardHeaders: true, legacyHeaders: false, message: { error: 'Too many requests, please slow down.' } });
/**
...
router.post('/generate', requireAuth, requireFeature('content_creation'), async (req, res) => {
  ...
  try {
    ...
    if (rubric_id && !rubric_context) {
      const q = isMySQL()
        ? await query('SELECT name, criteria, total_points FROM rubrics WHERE id = ? AND user_id = ?', [rubric_id, req.user.id])
        : await query('SELECT name, criteria, total_points FROM rubrics WHERE id = $1 AND user_id = $2', [rubric_id, req.user.id]);
      const row = Array.isArray(q) ? q[0] : (q.rows && q.rows[0]);
      if (row) {
...
router.post('/export/pptx', requireAuth, requireFeature('content_creation'), async (req, res) => {
  try {
    ...
    if (usedFallback) {
      res.setHeader('X-Acexen-Export-Fallback', 'local');
    }
    res.send(buffer);
  } catch (error) {
